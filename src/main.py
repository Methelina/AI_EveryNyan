        ...
        async for chunk in stream:
            ...
            lc_chunk = ChatGenerationChunk(
                message=AIMessageChunk(
                    content=content, additional_kwargs={"reasoning_content": reasoning}
                )
            )
            yield lc_chunk
    ...
    if _current_ai_message_tag and dpg.does_item_exist(_current_ai_message_tag):
        ...
        with dpg.group(parent="chat_area", horizontal=False):
            ...
            with dpg.group(indent=20):
                _current_ai_message_tag = dpg.add_text(
                    text, wrap=max(200, dpg.get_viewport_width() - 150)
                )
...
def init_components():
    ...
    logger.info(
        f"Chat mode: {settings.chat_mode}, endpoint: {chat_cfg.base_url}, model: {chat_cfg.chat_model}"
    )
    logger.info(
        f"Embedding mode: {settings.embedding_mode}, endpoint: {embed_cfg.base_url}, model: {embed_cfg.embedding_model}"
    )
...
async def query_memory(
    query: str, top_k: Optional[int] = None, filter_meta: Optional[Dict] = None
) -> str:
    if top_k is None:
    ...
    add_ai_thought(
        f'[RAG] QUERY: "{query[:60]}{"..." if len(query) > 60 else ""}" (k={top_k})',
        (180, 220, 255),
    )
    if filter_meta:
    ...
    try:
        ...
        add_ai_thought(
            f"[RAG] FOUND: {len(docs)} document(s) (requested {top_k})", (150, 255, 150)
        )
        ...
        for i, (doc, score) in enumerate(zip(docs, scores)):
            ...
            add_ai_thought(
                f"  [{i + 1}] score={score:.3f} | {snippet}...", (200, 200, 200)
            )
            content = doc.page_content[:800]
            formatted_memories.append(
                f"--- Воспоминание {i + 1} (релевантность {score:.2f}) ---\n{content}"
            )
...
async def check_plagiarism(text: str, threshold: float) -> bool:
    ...
    try:
        ...
        if results and results[0].score > threshold:
            add_ai_thought(
                f"[MEM] BLOCK: Duplicate (sim={results[0].score:.2f})", (255, 150, 150)
            )
            return True
        return False
    except Exception as e:
...
async def _extract_dialogue_metadata(
    user_text: str, ai_response: str
) -> Dict[str, Any]:
...
"""
    try:
        response = await llm.ainvoke(
            [
                ("system", "You are a metadata extractor. Output only JSON."),
                ("human", prompt),
            ]
        )
...
async def dump_context_to_memory():
    ...
    for i, msg in enumerate(session_context[:5]):
        add_ai_thought(
            f"  [{i}] {msg['role']}: {msg['content'][:40]}...", (150, 150, 180)
        )
    try:
        dialogue_text = "\n".join(
            [f"{msg['role']}: {msg['content']}" for msg in session_context]
        )
        full_prompt = [
        ...
        for idx, section in enumerate(sections):
            ...
            if await check_plagiarism(
                clean_section, settings.diary.plagiarism_threshold
            ):
                continue
            if query_preprocessor:
                lemmatized = query_preprocessor.lemmatize_text(
                    clean_section, remove_stopwords=False
                )
                parsed_meta.type_specific["lemmatized"] = lemmatized
            vector_store.add_texts(
                texts=[clean_section], metadatas=[parsed_meta.to_qdrant_payload()]
            )
            if memory_manager:
            ...
            saved_count += 1
        add_ai_thought(
            f"[DB] FINISH: {saved_count}/{total_sections} stored", (100, 255, 100)
        )
...
def check_anti_repetition_semantic(new_content: str) -> bool:
    ...
    try:
        ...
        if (
            max_sim > settings.anti_repeat.trigger_max
            or avg_sim > settings.anti_repeat.trigger_avg
        ):
            add_ai_thought(
                f"[ANTIREPEAT] BLOCKED: max={max_sim:.2f} avg={avg_sim:.2f}",
                (255, 200, 100),
            )
            return True
...
async def process_message(user_text: str) -> str:
    ...
    if not session_context and memory_manager:
        add_ai_thought(
            "[CTX] Context empty, loading recent history from DuckDB", (200, 150, 100)
        )
        fresh = memory_manager.get_recent_history(
    ...
    async def attempt_generation():
        add_ai_thought(
            f"[CTX] Current context size: {len(session_context)} messages",
            (150, 180, 200),
        )
        if len(session_context) > settings.context.warn_if_context_exceeds:
            add_ai_thought(
                f"[CTX] WARN: Large context ({len(session_context)}), may need dump soon",
                (255, 200, 100),
            )
        ...
        if not rag_context and memory_manager:
            add_ai_thought(
                "[RAG] No vectors, trying DuckDB keyword search", (200, 150, 100)
            )
    ...
    try:
        ...
        if "context length" in error_str or "exceeds" in error_str:
            add_ai_thought(
                f"[WARN] CONTEXT OVERFLOW: {len(session_context)} messages",
                (255, 150, 100),
            )
            ...
            if memory_manager:
                ...
                session_context.extend(fresh)
                add_ai_thought(
                    f"[CTX] Rehydrated: loaded {len(fresh)} messages", (150, 255, 150)
                )
            return await attempt_generation()
        elif "timeout" in error_str:
            add_ai_thought(
                "[WARN] LLM request timed out. Please try again with a shorter message.",
                (255, 200, 100),
            )
...
async def save_to_memory(user_text: str, ai_response: str):
    ...
    add_ai_thought(
        f"[CTX] Context size: {len(session_context)} messages", (150, 180, 200)
    )
    ...
    if vector_store:
        ...
        if query_preprocessor:
            lemmatized = query_preprocessor.lemmatize_text(
                content, remove_stopwords=False
            )
            meta["lemmatized"] = lemmatized
            add_ai_thought(
                f"[RAG] Lemmatized copy stored (length {len(lemmatized)})",
                (150, 200, 150),
            )
...
async def report_qdrant_status():
    ...
    try:
        ...
        add_ai_thought(
            f"[RAG] Qdrant collection '{settings.vector_db.collection}': {info.points_count} vectors",
            (150, 255, 150),
        )
...
def on_memory_report():
    ...
    if memory_manager:
        stats = memory_manager.get_stats()
        add_ai_thought(
            f"[DB] DuckDB: {stats.get('total_messages', 0)} msgs, {stats.get('total_summaries', 0)} summaries",
            (150, 255, 150),
        )
        ...
        if summaries:
            for s in summaries:
                add_ai_thought(
                    f"  - {s['timestamp']}: {s['text'][:80]}...", (180, 180, 180)
                )
    ...
    with dpg.window(
        ...
        with dpg.child_window(tag="chat_area", height=-200, border=False):
            if memory_manager:
                ...
                for msg in history:
                    color = (
                        (100, 200, 255) if msg["role"] == "user" else (255, 200, 100)
                    )
                    ...
                    with dpg.group(horizontal=False):
                        ...
                        with dpg.group(indent=20):
                            dpg.add_text(
                                msg["content"],
                                wrap=max(200, dpg.get_viewport_width() - 150),
                            )
                        dpg.add_spacer(height=5)
            else:
                dpg.add_text("Welcome to AI_EveryNyan!", color=(150, 150, 200))
        with dpg.child_window(
            tag="ai_thoughts_area", height=120, label="[SYSTEM] LOG", border=True
        ):
            dpg.add_text(
                "[SYSTEM] STATUS: Idle",
                tag="thoughts_placeholder",
                color=(100, 100, 100),
            )
        with dpg.group(horizontal=True):
            dpg.add_input_text(
...
def on_send_message(sender, app_data):
    ...
    add_ai_thought(
        f'[IN] User: "{user_text[:30]}{"..." if len(user_text) > 30 else ""}"',
        (150, 200, 255),
    )
